...
---[SHARED AND MENU] Receives zero or more integers, converts each one to its corresponding UTF-8 byte sequence and returns a string with the concatenation of all these sequences.
---
---[(View on wiki)](https://wiki.facepunch.com/gmod/utf8.char)
...
---[SHARED AND MENU] Returns the number of UTF-8 sequences in the given string between positions startPos and endPos (both inclusive). If it finds any invalid UTF-8 byte sequence, returns false as well as the position of the first invalid byte.
---
---[(View on wiki)](https://wiki.facepunch.com/gmod/utf8.len)
...
---[SHARED AND MENU] Returns the byte-index of the n'th UTF-8-character after the given startPos (nil if none). startPos defaults to 1 when n is positive and -1 when n is negative. If n is zero, this function instead returns the byte-index of the UTF-8-character startPos lies within.
---
---[(View on wiki)](https://wiki.facepunch.com/gmod/utf8.offset)
